...
// Tipos TypeScript para as fases
interface SetPhase {
  set: (string | number)[];
  rule: string;
  correct: (selection: (string | number)[]) => boolean;
}
...
type Phase = SetPhase | ScenarioPhase;
...
// Função para verificar se é uma fase de cenário
function isScenarioPhase(phase: Phase): phase is ScenarioPhase {
  return 'opcoes' in phase;
}

const Generalizacao: React.FC = () => {
  ...
  const handleVerify = () => {
    if (isScenario) {
      // Para fases de cenário, verificação simples
      const selectedOption = Array.from(selection)[0];
      const correct = selectedOption === currentPhase.resposta;
      ...
      if (correct) {
        if (step < phases.length) {
          ...
          setFeedback('Concluído! Você aplicou a mesma lógica em vários cenários.');
          setIsComplete(true);
        }
      } else {
        setFeedback('Ainda não. Lembre-se da regra principal.');
      }
    } else {
      // Para fases de conjunto, usar a função correct
      const correct = currentPhase.correct(Array.from(selection));
  ...
  const handleOptionClick = (value: string | number) => {
    if (isScenario) {
      // Para cenários, apenas uma seleção
      setSelection(new Set([value]));
    } else {
      // Para conjuntos, múltiplas seleções
  ...
  const renderOptions = () => {
    if (isScenario) {
      ...
      return shuffledOptions.map((option, index) => (
        <button
          key={index}
          className={`button ${selection.has(option) ? 'active' : ''}`}
      ...
      const shuffledSet = shuffle([...currentPhase.set]);
      return shuffledSet.map((item, index) => (
        <button
          key={index}
          className={`button ${selection.has(item) ? 'active' : ''}`}
  ...
  return (
    <section className="card">
      ...
      {isScenario ? (
        ...
        <div>
          <p><strong>{currentPhase.rule}</strong></p>
          <p className="muted">Conjunto: {currentPhase.set.join(', ')}</p>
        </div>
      )}
      ...
      <div style={{ marginTop: '14px' }}>
        <label htmlFor="reflexao">Reflexão (MAPEAR):</label>
        <textarea
          id="reflexao"
          className="input"
          rows={3}
